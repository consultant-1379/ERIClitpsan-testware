...
class TestCase(SanTest):
    ...
    def test(self):
        ...
        print "lookah",luns_in_model
        lun_size_fudge = 1   # we needs this for the discrepancy in boot-lun sizing
        # Verify the LUNs exist in the Storage Array
        for lun in luns_in_model.values():
            if lun.properties['bootable'] == "false":
                continue  # skip non-bootable LUNs
            lun_name = lun.properties['lun_name']
            lun_size = str(int(self.normalise_to_megabytes(lun.properties['size'])) + lun_size_fudge)
            self.info("Checking boot LUN %s", lun_name)
            for array_lun in luns_in_array.values():
                if array_lun['Name'] == lun_name:
                    # Assert that the size in the model is equal to the size on
                    # the SAN + the boot-lun fudge factor (currently 1MB)
                    self.info("Checking sizes match for model and SAN for boot LUN %s", lun_name)
                    array_lun_size = array_lun['LUN Capacity(Megabytes)']
                    assert(lun_size == array_lun_size)
